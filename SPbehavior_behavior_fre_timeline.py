...
def pre_data(file_path, start_time, delay):
    # pre data
    # file_path = csv_FN[0]
    A = np.zeros((14, 14))
    ...
    data = df2.iloc[start_time:start_time + delay, 1:2]
...
if __name__ == '__main__':
    ...
    for i in range(1, 7):
        ...
        B = np.zeros((14, 14))
        for start_time in range(0, 18000, 9000):
            for item in csv_FD:
                a = pre_data(item, start_time + 5, 8995)
    ...
    #     # data_list['time'] = [i for i in range(1, 61)]
    #     # data_list["species"] = ['{}'.format(name)] * 60
    #     data_list['time'] = [i for i in range(1, 61, 10)]
    #     data_list["species"] = ['{}'.format(name)] * 6
    #     return data_list
    #
    #
    # male_night_data = pre_data(output_data1, 'male_night')
    #
    ...
    # sns.lineplot(data=male_night_data, x="time", y="mean", hue="species")
    #
    ...
    #     # end_time = 18000
    #     for item in csv_FD:
    #         for start_time in range(0, 18000, 1800):
    #             data_a = pre_data(item, start_time + 5, 1795)
    #             # if not np.isnan(a.all()):
    #             #     # print(a, item)
    #             # del_data, names, colors = del_pre_data(data_a)
    #             output_data.append(data_a.diagonal())
    #             # output_data.append([0]*14)
    #
    #     output_data1 = pd.DataFrame(output_data)
    #     file_path = 'D:/3D_behavior/Spontaneous_behavior/result/analysis_result/state_convert/v2/{}{}_all_{}_1min.csv'.format(gender, ExperimentTime, len(csv_FD))
    #     output_data1.to_csv(file_path)
    #     A = A + a
    # A = A / len(csv_FD)
